...
private const val THREADS_OEMBED_ENDPOINT = "https://graph.threads.com/oembed"
private const val THREADS_DOCUMENT_BASE_URL = "https://www.threads.com/"
...
class ThreadsProvider(
    ...
    override fun supports(uri: Uri): Boolean =
        ThreadsUrlPolicy.supports(uri.scheme, uri.host, uri.path)

    override fun resolve(uri: Uri): SocialContent {
        val canonical = ThreadsUrlPolicy.canonicalUrl(
            scheme = uri.scheme,
            host = uri.host,
            path = uri.path,
        ) ?: error("URL Threads non supportata")

        return resolveCanonicalThreads(
...
internal fun resolveCanonicalThreads(
    canonicalUrl: String,
    http: UrlConnectionHttpClient,
): SocialContent {
    val encodedUrl = URLEncoder.encode(canonicalUrl, StandardCharsets.UTF_8.name())
    val response = http.get("$THREADS_OEMBED_ENDPOINT?url=$encodedUrl")

    if (response.statusCode == 400 || response.statusCode == 404) {
        throw ProviderContentUnavailableException(
            providerName = "Threads",
            technicalDetail = "Threads oEmbed ha risposto HTTP ${response.statusCode}",
        )
    }
    if (response.statusCode !in 200..299) {
        error("Threads oEmbed ha risposto HTTP ${response.statusCode}")
    }

    val responseHost = runCatching {
        URI(response.finalUrl).host?.lowercase()
    }.getOrNull()
    require(responseHost == "graph.threads.com") {
        "Threads oEmbed ha reindirizzato fuori da Meta"
    }

    val json = JSONObject(response.body)
    val providerHtml = json.optString("html").takeIf { it.isNotBlank() }
        ?: error("Threads oEmbed non ha restituito markup incorporabile")
    ...
    return SocialContent(
        ...
        documentBaseUrl = THREADS_DOCUMENT_BASE_URL,
        embedHtml = threadsDocument(providerHtml),
    )
}

internal fun threadsDocument(providerHtml: String): String = """
    <!doctype html>
    <html>
      <head>
